classdef PoseGraph < handle
    ...
    methods
        ...
        function readGraph(obj, vfile, efile)
            ...
            % 裝node節點資訊的file: 總共有4列: id, x, y, yaw
            vfid = fopen(vfile);            
            if (vfid < 0)
        ...
        function linearize(obj)
            ...
            for i_edge = 1:obj.n_edge
                ...
                T_i = v2t(v_i);
                T_j = v2t(v_j);
                R_i = T_i(1:2,1:2); % i節點 -> 地    的旋轉矩陣
                R_z = T_z(1:2,1:2); % j節點 -> i節點 的旋轉矩陣
                
                si = sin(v_i(3));
                ci = cos(v_i(3));
                dR_i = [-si ci; -ci -si]';    % i節點 -> 地    的旋轉矩陣微分
                dt_ij = v_j(1:2) - v_i(1:2);  % j節點 -> i節點 邊向量 : ij向量
